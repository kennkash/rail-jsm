...
type RequestTypesApiResponse = {
  requestTypes?: RequestTypeOption[]; // optional/legacy
  groups?: Array<{
    id: string;
    name: string;
    displayOrder?: number;
...
const defaultRequestTypes: RequestTypeOption[] = [
  { id: "help", name: "Get Help", description: "Ask a question or get support", group: "Support", groups: ["Support"] },
  { id: "incident", name: "Report an Incident", description: "Report a problem or issue", group: "Support", groups: ["Support"] },
  { id: "question", name: "Ask a Question", description: "General questions and inquiries", group: "Support", groups: ["Support"] },
  { id: "service-request", name: "Service Request", description: "Request a service or item", group: "Services", groups: ["Services"] },
  { id: "access-request", name: "Access Request", description: "Request access to systems or resources", group: "Services", groups: ["Services"] },
  { id: "equipment", name: "Equipment Request", description: "Request equipment or supplies", group: "Services", groups: ["Services"] },
  { id: "change", name: "Request a Change", description: "Propose a change or improvement", group: "Changes", groups: ["Changes"] },
  { id: "feature", name: "Feature Request", description: "Suggest a new feature", group: "Changes", groups: ["Changes"] },
];
...
export function PortalRequestTypes(
  ...
  // ✅ Build grouped sections from API groups (already ordered by backend)
  const groupedSections = useMemo(() => {
    const apiGroups = data?.groups ?? [];
    if (apiGroups.length) {
      const orderedGroups = [...apiGroups].sort((a, b) => {
        const oa = typeof a.displayOrder === "number" ? a.displayOrder : Number.MAX_SAFE_INTEGER
        const ob = typeof b.displayOrder === "number" ? b.displayOrder : Number.MAX_SAFE_INTEGER
        return oa - ob
      })

      return orderedGroups
        .map((g) => {
          const types = (g.requestTypes ?? []).map((t) => {
            const iconUrlFromJsm = jsmIconUrls?.[t.id];
            return {
              ...t,
              iconUrl: iconUrlFromJsm || t.iconUrl,
              group: g.name, // ensure context
              displayOrder: typeof t.displayOrder === "number" ? t.displayOrder : undefined,
            } as RequestTypeOption
          })
          return { name: g.name, types }
        })
        .filter((s) => s.types.length)
    }

    // Fallback: old shape / configured defaults
    const fallbackRequestTypes = (data?.requestTypes?.length ? data.requestTypes : configuredRequestTypes).map((t) => {
      const iconUrlFromJsm = jsmIconUrls?.[t.id];
      return { ...t, iconUrl: iconUrlFromJsm || t.iconUrl }
    })

    // group by group name (legacy)
    const grouped = fallbackRequestTypes.reduce<Record<string, RequestTypeOption[]>>((acc, type) => {
      const groupName = (type.group || (type.groups?.[0] ?? "Other")) ?? "Other"
      acc[groupName] ||= []
      acc[groupName].push(type)
      return acc
    }, {})

    return Object.keys(grouped).map((name) => ({ name, types: grouped[name] }))
  }, [data, configuredRequestTypes, jsmIconUrls]);

  // Flat requestTypes for deep-link searching
  const requestTypes = useMemo(() => groupedSections.flatMap((s) => s.types), [groupedSections]);
  ...
  const handleSubmissionSuccess = useCallback(
    (href: string | null, issueKey: string | null, requestDetails?: { name: string; description?: string | null }) => {
  ...
  const buildRequestUrl = useCallback((requestTypeId: string) => {
    if (!enableInteractivePreview) return null;
    const base = getBootstrapBaseUrl();
    if (!base) return null;
  ...
  }, [enableInteractivePreview, resolvedPortalId, resolvedServiceDeskId]);

  const handleRequestTypeClick = useCallback((requestType: RequestTypeOption) => {
    if (!enableInteractivePreview) return;
  ...
  useEffect(() => {
    if (!enableInteractivePreview || !routeRequestTypeId || !requestTypes?.length) return;
  ...
  const parseSuccessFromHref = (href: string | null) => {
    ...
    try {
      ...
      return { issueKey: looksLikeIssueKey ? issueKeyCandidate : null, success: !isCreatePath && looksLikeIssueKey };
    } catch (e) {
  ...
  const renderTypeAvatar = (type: RequestTypeOption, className?: string, sizeClass = "h-8 w-8") => {
    const initial = type.name?.charAt(0)?.toUpperCase() ?? "?";
    return (
      <Avatar className={cn(sizeClass, className, "!rounded-none")}>
        {type.iconUrl ? <AvatarImage src={type.iconUrl} alt={`${type.name} icon`} /> : null}
        <AvatarFallback className="text-xs font-medium !rounded-none">{initial}</AvatarFallback>
      </Avatar>
    );
  };
  ...
  const RequestTypeCard = ({ type, index }: { type: RequestTypeOption; index: number }) => {
    return (
      <Card
        ...
        className={cn("h-full cursor-pointer transition-all duration-200 hover:border-border hover:shadow-md")}
        onClick={() => handleRequestTypeClick(type)}
      >
        <CardHeader className="space-y-2">
          <div className="flex items-center gap-4">
            <div className="mt-1">{renderTypeAvatar(type, undefined, "h-7 w-7")}</div>
            <CardTitle className="text-lg font-semibold text-gray-900 text-left">{type.name}</CardTitle>
          </div>
          {type.description ? (
  ...
  const renderRequestTypes = (types: RequestTypeOption[]) => {
    ...
    // ✅ Do NOT sort here — backend already sends correct order per group
          ...
          {types.map((type) => (
            <AccordionItem key={type.id} value={type.id}>
              ...
              <AccordionContent className="px-3 pb-3">
                <Button type="button" size="sm" className="mt-1 cursor-pointer" onClick={() => handleRequestTypeClick(type)}>
    ...
    return (
      <div
        className={cn(layout === "grid" ? "grid gap-4" : "flex flex-col gap-4")}
        style={
          layout === "grid"
  ...
  const frameContainerClass = iframeStatus === "ready" ? "h-[750px]" : "flex-1 min-h-[320px] sm:min-h-[380px]";
        ...
        <Dialog
          ...
          onOpenChange={(open) => {
            ...
            if (!open && onRouteChange && routeProjectKey) {
              const basePath = buildRoutePath(ROUTE_PATHS.CUSTOMER_PORTAL_PROJECT, { projectKey: routeProjectKey });
        ...
        >
          <DialogContent className={cn("sm:max-w-[800px] flex flex-col gap-4", dialogMaxHeightClass)}>
            {postSubmitUrl ? (
              <div className="space-y-4">
                ...
                {lastSubmittedRequest && (
                  <div className="rounded-md border bg-muted/40 p-4 space-y-2">
                    <p className="text-sm font-semibold text-foreground">{lastSubmittedRequest.name}</p>
                    {lastSubmittedRequest.description && (
                      <p className="text-sm text-muted-foreground">{lastSubmittedRequest.description}</p>
                    )}
                  </div>
                )}
                <div className="grid gap-3 sm:grid-cols-2">
                  <Button variant="default" className="w-full cursor-pointer" onClick={() => window.open(postSubmitUrl, "_blank", "noopener")}>
                    View request
                  </Button>
                  <Button variant="secondary" className="w-full cursor-pointer" onClick={() => window.location.reload()}>
                    Submit another {selectedRequestType?.name ?? "request"}
                  </Button>
                </div>
              </div>
            ) : iframeUrl ? (
              <div className={cn("relative rounded-md border overflow-hidden w-full", frameContainerClass, iframeStatus !== "ready" && "max-h-[60vh]")}>
                {iframeStatus === "loading" && (
                  <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-3 bg-background/90 backdrop-blur-sm">
                    ...
                    <p className="text-sm text-muted-foreground">Loading {loadingLabel}...</p>
                  </div>
                )}
                <iframe
                  key={iframeUrl}
              ...
              <p className="text-sm text-muted-foreground">Unable to open the request form. Please try again later.</p>
            )}
...
function RequestTypesOptionsGroup() {
  ...
  const showGroupsValue = (selectedComponent.getField("properties.showGroups") ?? "yes") !== "no";
  const variant = (selectedComponent.getField("properties.variant") as string | undefined) || "cards";
  const layout = selectedComponent.getField("properties.layout") || "grid";
  const groupLayout =
    (selectedComponent.getField("properties.groupLayout") as string | undefined) || "top-tabs";
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        <div className={cn("relative rounded-md", isGeneratingSubtitle && "ring-2 ring-primary/50 ring-offset-1 animate-pulse")}>
          <Textarea
      ...
      <div className="space-y-2">
        ...
        <Select value={variant} onValueChange={(value) => handleChange("properties.variant", value)}>
      ...
      {variant === "cards" && (
        <>
          <div className="space-y-2">
            ...
            <Select value={layout} onValueChange={(value) => handleChange("properties.layout", value)}>
              <SelectTrigger className="text-sm"><SelectValue /></SelectTrigger>
              <SelectContent>
      ...
      <div className="flex items-center space-x-2 pt-2">
        <Checkbox
          ...
          onCheckedChange={(checked) => handleChange("properties.showGroups", checked ? "yes" : "no")}
        />
        <Label htmlFor="showGroups" className="text-sm font-medium leading-none cursor-pointer">
          Group by Category
        </Label>
      </div>
      ...
      {showGroupsValue && (
        <div className="space-y-2 pl-6 border-l-2 border-muted mt-4">
          ...
          <ToggleGroup
            type="single"
            value={(selectedComponent.getField("properties.groupLayout") as string | undefined) || "top-tabs"}
            onValueChange={(value) => { if (value) handleChange("properties.groupLayout", value); }}
            className="flex w-full gap-1"
          >
            <ToggleGroupItem value="top-tabs" className="flex-1 text-xs">Top Tabs</ToggleGroupItem>
            <ToggleGroupItem value="sectioned" className="flex-1 text-xs">Sectioned</ToggleGroupItem>
          </ToggleGroup>
        </div>
      )}
      ...
      {adminUrl && (
        <div className="space-y-2 border-t pt-4">
          ...
          <Button variant="outline" size="sm" asChild>
            <a href={adminUrl} target="_blank" rel="noreferrer">Open Request Types</a>
          </Button>
