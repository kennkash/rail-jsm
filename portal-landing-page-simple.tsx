...
export function PortalLandingPageSimple() {
  ...
  const handleSelect = useCallback((portal: PortalInfo) => {
            ...
            <Tabs value={activeTab} onValueChange={(val) => setActiveTab(val as typeof activeTab)}>
              ...
              <TabsContent value="recent" className="mt-4">
                ...
                {!isLoading && recentPortals.length === 0 && (
                  <EmptyState
                    icon={Clock}
                    title="No recently visited portals"
                    subtitle="Visit a portal to see it here"
                  />
                )}
...
function CategoryButton({
  ...
  return (
    <button
      type="button"
      className={cn(
        "w-full text-left rounded-md px-3 py-2 text-sm transition-colors cursor-pointer",
        isActive
...
function PortalGrid({
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {portals.map((portal) => (
        <button
          key={`${portal.projectKey}-${portal.portalId ?? portal.jsmPortalId ?? "portal"}`}
          type="button"
          onClick={() => onSelect(portal)}
          className="text-left rounded-lg border bg-card p-4 transition-colors hover:bg-muted cursor-pointer"
        >
          <div className="font-medium text-sm">{portal.projectName}</div>
          <div className="text-xs text-muted-foreground mt-1">{portal.projectKey}</div>
          {portal.categoryName && (
            <div className="text-xs text-muted-foreground mt-2">{portal.categoryName}</div>
          )}
        </button>
      ))}
    </div>
