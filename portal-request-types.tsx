...
export function PortalRequestTypes(
  ...
  const apiRequestTypes = useMemo<RequestTypeOption[] | null>(() => {
    ...
    const mappedTypes = data.requestTypes.map((type) => {
      ...
      return {
        ...
        displayOrder: type.displayOrder ?? 0, // Ensure displayOrder is always a number
      };
    });

    // Sort by displayOrder, then by name as fallback
    return [...mappedTypes].sort((a, b) => {
      // Ensure both have numeric displayOrder
  ...
  const groupedTypes = requestTypes.reduce<Record<string, RequestTypeOption[]>>(
    (acc, type) => {
      const targetGroups = (type.groups && type.groups.length > 0
        ? type.groups
        : type.group
          ? [type.group]
          : ["Other"]).map((groupName) => groupName || "Other");
  ...
  const fallbackGroups = Object.keys(groupedTypes);
  const availableGroups = apiGroupNames.length
    ? [...apiGroupNames, ...fallbackGroups.filter((group) => !apiGroupNames.includes(group))]
    : fallbackGroups;
  const groupedSections = availableGroups
    .filter((group) => groupedTypes[group]?.length)
    .map((group) => ({
      name: group,
